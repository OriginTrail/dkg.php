...
class FixedSizeTree
{
     ...
     */
    public function __construct(int $width, callable $hasher, $options = null, callable $finished = null)
    {
        if ($width < 1) {
        ...
        $this->tree = $tree;
        $this->hasher = $hasher;
        $this->sortPairs = isset($options) ? $options->sortPairs : false;
    ...
    {
        $result = $this->tree->set($i, call_user_func($this->hasher, $v), $this->sortPairs);
        if ($result === null) {
            return null;
        }
