...
class GrowableBinaryTree
{
     ...
     * @param int $i
     * @param string $v
     * @param bool $sortPairs
     * @return string|null
     */
    public function set(int $i, string $v, bool $sortPairs)
    {
        ...
        return $this->setOnRow($i, $v, 0, $sortPairs);
    }
     ...
     * @return string|null
     */
    private function setOnRow(int $i, string $v, int $row, bool $sortPairs): ?string
    {
...
//            $newHash = call_user_func($this->hasher, $v . $v);
            $parentIndex = (int) ($i / 2);
            return $this->setOnRow($parentIndex, $v, $row + 1, $sortPairs);
        }
        ...
        if($sortPairs) {
            $combined = [$v, $sibling];
            sort($combined);
            $parentHashData = $combined[0] . $combined[1];
        } else {
            $parentHashData = $oddIdx ? $sibling . $v : $v . $sibling;
        }
        $newHash = call_user_func($this->hasher, $parentHashData);
        $parentIndex = (int) ($i / 2);
        return $this->setOnRow($parentIndex, $newHash, $row + 1, $sortPairs);
    }
}
